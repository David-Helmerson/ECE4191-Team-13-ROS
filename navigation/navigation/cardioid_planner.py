...
class CardioidNode(Node):
    ...
    def __init__(self):
        ...
        # Important objects
        self.goal_wp = None
        self.planner = CardioidPlanner(2/self.freq, 1, 1, 0)  # Plans head for twice the node's operating freq, just incase
        ...
        self.planner_timer = self.create_timer(1/self.freq, self.planner_callback)
    ...
    def planner_callback(self):
        if self.goal_wp is not None:
            pose = self.get_pose()
            v, w = self.planner.plan_linag(self.goal_wp.x, self.goal_wp.y, 0, pose.x, pose.y, pose.th)
